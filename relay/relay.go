...
// Defaults
const (
	defaultAPIHost = "api.relay.wallet.openweb3.io"
	apiPrefix      = "api/v1"
...
func NewClient(token string, localURL *url.URL, opts *ClientOptions) *Client {
	...
	return &Client{
		...
		localURL:           localURL,
		receiveURLTemplate: "https://play.openweb3.io/in/%s/",
		dialer: &websocket.Dialer{
...
`, pretty.MakeTerminalLink(url, url), c.localURL)
	if c.logging {
		viewUrl := fmt.Sprintf("https://play.openweb3.io/view/%s/", c.token)
		fmt.Printf(`
View logs and debug information at
%s
To disable logging run "openwallet listen --no-logging"
`,
